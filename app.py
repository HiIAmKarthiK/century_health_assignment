import findspark
findspark.init()
from pyspark.sql import SparkSession
from pyspark.sql.functions import *
from datetime import datetime
import matplotlib.pyplot as plt
import psycopg2
from enum import Enum


class DB(Enum):
    BRONZE='bronze'
    SILVER='silver'
    GOLD='gold'

class TABLE(Enum):
    PATIENTS='patients'
    CONDITIONS='conditions'
    MEDICATIONS='medications'
    ENCOUNTERS='encounters'
    SYMPTOMS='symptoms'

urls={
    "bronze":"jdbc:postgresql://localhost:5432/bronze",
    "silver":"jdbc:postgresql://localhost:5432/silver",
    "gold":"jdbc:postgresql://localhost:5432/gold",
    "postgres" : "jdbc:postgresql://localhost:5432/postgres"
}

properties = {
    "user": "airflow",
    "password": "airflow",
    "driver": "org.postgresql.Driver"
    }




def main():

    global spark, ingest_datetime

    # Initialize Spark session
    spark = SparkSession.builder \
    .config("spark.jars", "D:\\personal\\century_health_assignment\\jars\\postgresql-42.7.3.jar")\
    .appName("patients_data_analysis_app") \
    .getOrCreate()

    ingest_datetime = datetime.utcnow()

    clean()

def connect():

    # connect to source database in our case, we are using files
    patients_df = spark.read.csv(header=True,path='source_data/patients.csv')
    conditions_df = spark.read.csv(header=True,path='source_data/conditions.csv')
    medications_df = spark.read.csv(header=True,path='source_data/medications.csv')
    encounters_df = spark.read.parquet('source_data/encounters.parquet')
    symptoms_df = spark.read.csv(header=True,path='source_data/symptoms.csv')
    patients_gender_df = spark.read.csv(header=True,path='source_data/patients_gender.csv')

    patients_df.printSchema()
    encounters_df.printSchema()
    conditions_df.printSchema()
    medications_df.printSchema()
    symptoms_df.printSchema()

    create_table(DB.BRONZE.value)
    create_table(DB.SILVER.value)
    create_table(DB.GOLD.value)

    patients_df = patients_df.select(
    patients_df['PATIENT_ID'].alias('patient_id'),
    patients_df['BIRTHDATE'].alias('birth_date'),
    patients_df['DEATHDATE'].alias('death_date'),
    patients_df['SSN'].alias('social_security_number'),
    patients_df['FIRST'].alias('first_name'),
    patients_df['LAST'].alias('last_name'),
    patients_df['RACE'].alias('race'),
    patients_df['ADDRESS'].alias('address'),
    patients_df['CITY'].alias('city'),
    patients_df['STATE'].alias('state'),
    patients_df['COUNTY'].alias('county'),
    patients_df['ZIP'].alias('zip_code'),
    patients_df['LAT'].alias('latitude'),
    patients_df['LON'].alias('longitude'),
    )

    patients_df.write.jdbc(url=urls['bronze'], table=TABLE.PATIENTS.value, mode="append", properties=properties)

    conditions_df = conditions_df.select(
    conditions_df['START'].alias('onset_date'),
    conditions_df['STOP'].alias('resolved_date'),
    conditions_df['PATIENT'].alias('patient_id'),
    conditions_df['ENCOUNTER'].alias('encounter_id'),
    conditions_df['CODE'].alias('source_code'),
    conditions_df['DESCRIPTION'].alias('source_description'))

    conditions_df.write.jdbc(url=urls['bronze'], table=TABLE.CONDITIONS.value, mode="append", properties=properties)

    medications_df = medications_df.select(
    medications_df['START'].alias('medication_start_date'),
    medications_df['STOP'].alias('medication_end_date'),
    medications_df['PATIENT'].alias('patient_id'),
    medications_df['ENCOUNTER'].alias('encounter_id'),
    medications_df['CODE'].alias('source_code'),
    medications_df['DESCRIPTION'].alias('source_description'))

    medications_df.write.jdbc(url=urls['bronze'], table=TABLE.MEDICATIONS.value, mode="append", properties=properties)

    encounters_df = encounters_df.select(
    encounters_df['START'].alias('encounter_start_date'),
    encounters_df['STOP'].alias('encounter_end_date'),
    encounters_df['PATIENT'].alias('patient_id'),
    encounters_df['id'].alias('encounter_id'),
    encounters_df['CODE'].alias('admit_type_code'),
    encounters_df['DESCRIPTION'].alias('admit_type_description'))

    encounters_df.write.jdbc(url=urls['bronze'], table=TABLE.ENCOUNTERS.value, mode="append", properties=properties)

    symptoms_df = symptoms_df.select(
    symptoms_df['PATIENT'].alias('patient_id'),
    symptoms_df['AGE_BEGIN'].alias('age'),
    symptoms_df['PATHOLOGY'].alias('pathology'),
    symptoms_df['NUM_SYMPTOMS'].alias('number_of_symptoms'),
    symptoms_df['SYMPTOMS'].alias('symptoms'))

    symptoms_df.write.jdbc(url=urls['bronze'], table=TABLE.SYMPTOMS.value, mode="append", properties=properties)

    return patients_df,conditions_df,medications_df,encounters_df,symptoms_df,patients_gender_df


def clean():

    patients_df,conditions_df,medications_df,encounters_df,symptoms_df,patients_gender_df =  connect()

    patients_gender_df = patients_gender_df.withColumnRenamed('id','patient_id') \
    .withColumnRenamed('GENDER','gender')

    patients_df = patients_df.join(patients_gender_df,['patient_id'],'left')

    patients_df = patients_df.withColumn('birth_date',to_date(col("birth_date"), "yyyy-MM-dd"))\
    .filter(col('birth_date').isNotNull()) \
    .filter(col('zip_code') != lit('0'))

    patients_df.write.jdbc(url=urls['silver'], table=TABLE.PATIENTS.value, mode="append", properties=properties)
    conditions_df.write.jdbc(url=urls['silver'], table=TABLE.CONDITIONS.value, mode="append", properties=properties)
    symptoms_df.write.jdbc(url=urls['silver'], table=TABLE.SYMPTOMS.value, mode="append", properties=properties)

    medications_df = medications_df.withColumn('medication_start_date',to_timestamp(col("medication_start_date"), "yyyy-MM-dd'T'HH:mm:ss'Z'")) \
    .withColumn('medication_end_date',to_timestamp(col("medication_end_date"), "yyyy-MM-dd'T'HH:mm:ss'Z'"))

    medications_df.write.jdbc(url=urls['silver'], table=TABLE.MEDICATIONS.value, mode="append", properties=properties)

    encounters_df = encounters_df.withColumn('encounter_start_date',to_timestamp(col("encounter_start_date"), "yyyy-MM-dd'T'HH:mm:ss'Z'")) \
    .withColumn('encounter_end_date',to_timestamp(col("encounter_end_date"), "yyyy-MM-dd'T'HH:mm:ss'Z'"))

    encounters_df.write.jdbc(url=urls['silver'], table=TABLE.ENCOUNTERS.value, mode="append", properties=properties)

    structure(patients_df,conditions_df,medications_df,encounters_df,symptoms_df)

    print('distinct patients -----------------')

    patients_count = patients_df.select('patient_id').distinct().count()
    print(patients_count)

    df_grouped = patients_df.groupBy("race", "gender").agg(count("patient_id").alias("patient_count"))

    df_grouped = df_grouped.withColumn("percentage", (col("patient_count") / patients_count) * 100)

    df_grouped.show()

    # Convert the PySpark DataFrame to Pandas DataFrame for plotting
    pandas_df = df_grouped.toPandas()

    pandas_df['label'] = pandas_df['race'] + " - " + pandas_df['gender']

    # Plotting the pie chart
    plt.figure(figsize=(14, 8))
    plt.pie(
    pandas_df['percentage'],
    labels=pandas_df['label'],
    autopct='%1.1f%%',
    startangle=140
)
    plt.title('Percentage of Patients by Racial Category and Gender')
    plt.show()

    # What percentage of patients have all 4 symptom categories ≥ 30?

    symptoms_df_p = symptoms_df.filter((col('number_of_symptoms') == lit(4)) & (col('age') >= lit(30)))

    conditional_symptoms = symptoms_df_p.select('patient_id').distinct().count()
    
    print("result ",(conditional_symptoms / patients_count) * 100)

def structure(patients_df,conditions_df,medications_df,encounters_df,symptoms_df):

    patients_df.write.jdbc(url=urls['gold'], table=TABLE.PATIENTS.value, mode="append", properties=properties)
    conditions_df.write.jdbc(url=urls['gold'], table=TABLE.CONDITIONS.value, mode="append", properties=properties)
    encounters_df.write.jdbc(url=urls['gold'], table=TABLE.ENCOUNTERS.value, mode="append", properties=properties)
    medications_df.write.jdbc(url=urls['gold'], table=TABLE.MEDICATIONS.value, mode="append", properties=properties)

    symptoms_df_mod = symptoms_df \
    .withColumn("symptom_array", split(symptoms_df["symptoms"], ";")) \
    .withColumn("symptom_ele", explode("symptom_array")) \
    .withColumn("symptom_name", regexp_extract("symptom_ele", r"([^:]+)", 1)) \
    .withColumn("symptom_value", regexp_extract("symptom_ele", r"(\d+)", 0))

    symptoms_df_mod = symptoms_df_mod.groupBy("patient_id").pivot("symptom_name").agg(first("symptom_value")) \
    .withColumnRenamed('Fatigue','fatigue_level')\
    .withColumnRenamed('Fever','fever_level')\
    .withColumnRenamed('Joint Pain','joint_pain_level')\
    .withColumnRenamed('Rash','rash_level')

    symptoms_df = symptoms_df.join(symptoms_df_mod,['patient_id']).drop('symptoms')

    symptoms_df.write.jdbc(url=urls['gold'], table=TABLE.SYMPTOMS.value, mode="append", properties=properties)

    merge(patients_df,conditions_df,medications_df,encounters_df,symptoms_df)

def merge(patients_df,conditions_df,medications_df,encounters_df,symptoms_df):

    patients_df = patients_df.withColumn('patient_id',lower('patient_id'))
    encounters_df = encounters_df.withColumn('patient_id',lower('patient_id')) \
    .withColumn('encounter_id',lower('encounter_id')) 
    symptoms_df = symptoms_df.withColumn('patient_id',lower('patient_id'))
    medications_df = medications_df.withColumn('patient_id',lower('patient_id')) \
    .withColumn('encounter_id',lower('encounter_id')) 


    final_df = patients_df.join(encounters_df,['patient_id'],'left') \
    .join(conditions_df,['patient_id'],'left') \
    .join(symptoms_df,['patient_id'],'left')\
    .join(medications_df,['patient_id'],'left')

    final_df.show()





def create_table(db_name):

    if db_name == 'bronze':

        conn = psycopg2.connect(
        host="localhost",
        database=db_name,
        user="dev_user",
        password="airflow"
        )

        # Create a cursor object
        cur = conn.cursor()

        table_schemas = ["""
        CREATE TABLE IF NOT EXISTS patients (
        patient_id VARCHAR(50) PRIMARY KEY,
        birth_date VARCHAR(20),
        death_date VARCHAR(20),
        social_security_number VARCHAR(50),
        first_name VARCHAR(50),
        last_name VARCHAR(50),
        race VARCHAR(20),
        address VARCHAR(255),
        city VARCHAR(100),
        state VARCHAR(20),
        county VARCHAR(20),
        zip_code VARCHAR(10),
        latitude VARCHAR(20),
        longitude VARCHAR(20)
        );
        """,
        """CREATE TABLE IF NOT EXISTS conditions (
        onset_date VARCHAR(20),
        resolved_date VARCHAR(20),
        patient_id VARCHAR(255),
        encounter_id VARCHAR(255),
        source_code VARCHAR(50),
        source_description TEXT
        );""",
        """CREATE TABLE IF NOT EXISTS medications (
        medication_start_date VARCHAR(20),
        medication_end_date VARCHAR(20),
        patient_id VARCHAR(255),
        encounter_id VARCHAR(255),
        source_code VARCHAR(20),
        source_description VARCHAR(50)
        );""",
        """CREATE TABLE IF NOT EXISTS encounters (
        encounter_start_date VARCHAR(20),
        encounter_end_date VARCHAR(20),
        patient_id VARCHAR(255),
        encounter_id VARCHAR(255),
        admit_type_code VARCHAR(20),
        admit_type_description VARCHAR(255)
        );""",
        """CREATE TABLE IF NOT EXISTS symptoms (
        patient_id VARCHAR(255),
        age VARCHAR(50),
        pathology VARCHAR(255),
        number_of_symptoms VARCHAR(50),
        symptoms VARCHAR(255)
        );"""]

        for schema in table_schemas:
            cur.execute(schema)

        # Commit changes
        conn.commit()

        # Close the connection
        cur.close()
        conn.close()


    if db_name == 'silver':

        conn = psycopg2.connect(
        host="localhost",
        database=db_name,
        user="dev_user",
        password="airflow"
        )

        # Create a cursor object
        cur = conn.cursor()

        table_schemas = ["""
        CREATE TABLE IF NOT EXISTS patients (
        patient_id VARCHAR(50) PRIMARY KEY,
        birth_date VARCHAR(20),
        death_date VARCHAR(20),
        social_security_number VARCHAR(50),
        first_name VARCHAR(50),
        last_name VARCHAR(50),
        race VARCHAR(20),
        address VARCHAR(255),
        city VARCHAR(100),
        state VARCHAR(20),
        county VARCHAR(20),
        zip_code VARCHAR(10),
        latitude VARCHAR(20),
        longitude VARCHAR(20),
        gender VARCHAR(20)
        );
        """,
        """CREATE TABLE IF NOT EXISTS conditions (
        onset_date VARCHAR(20),
        resolved_date VARCHAR(20),
        patient_id VARCHAR(255),
        encounter_id VARCHAR(255),
        source_code VARCHAR(50),
        source_description TEXT
        );""",
        """CREATE TABLE IF NOT EXISTS medications (
        medication_start_date VARCHAR(50),
        medication_end_date VARCHAR(50),
        patient_id VARCHAR(255),
        encounter_id VARCHAR(255),
        source_code VARCHAR(50),
        source_description VARCHAR(255)
        );""",
        """CREATE TABLE IF NOT EXISTS encounters (
        encounter_start_date VARCHAR(50),
        encounter_end_date VARCHAR(50),
        patient_id VARCHAR(255),
        encounter_id VARCHAR(255),
        admit_type_code VARCHAR(50),
        admit_type_description VARCHAR(255)
        );""",
        """CREATE TABLE IF NOT EXISTS symptoms (
        patient_id VARCHAR(255),
        age VARCHAR(50),
        pathology VARCHAR(255),
        number_of_symptoms VARCHAR(50),
        symptoms VARCHAR(255)
        );"""]

        for schema in table_schemas:
            cur.execute(schema)

        # Commit changes
        conn.commit()

        # Close the connection
        cur.close()
        conn.close()

    if db_name == 'gold':

        conn = psycopg2.connect(
        host="localhost",
        database=db_name,
        user="dev_user",
        password="airflow"
        )

        # Create a cursor object
        cur = conn.cursor()

        table_schemas = ["""
        CREATE TABLE IF NOT EXISTS patients (
        patient_id VARCHAR(50) PRIMARY KEY,
        birth_date VARCHAR(20),
        death_date VARCHAR(20),
        social_security_number VARCHAR(50),
        first_name VARCHAR(50),
        last_name VARCHAR(50),
        race VARCHAR(20),
        address VARCHAR(255),
        city VARCHAR(100),
        state VARCHAR(20),
        county VARCHAR(20),
        zip_code VARCHAR(10),
        latitude VARCHAR(20),
        longitude VARCHAR(20),
        gender VARCHAR(20)
        );
        """,
        """CREATE TABLE IF NOT EXISTS conditions (
        onset_date VARCHAR(20),
        resolved_date VARCHAR(20),
        patient_id VARCHAR(255),
        encounter_id VARCHAR(255),
        source_code VARCHAR(50),
        source_description TEXT
        );""",
        """CREATE TABLE IF NOT EXISTS medications (
        medication_start_date VARCHAR(50),
        medication_end_date VARCHAR(50),
        patient_id VARCHAR(255),
        encounter_id VARCHAR(255),
        source_code VARCHAR(50),
        source_description VARCHAR(255)
        );""",
        """CREATE TABLE IF NOT EXISTS encounters (
        encounter_start_date VARCHAR(50),
        encounter_end_date VARCHAR(50),
        patient_id VARCHAR(255),
        encounter_id VARCHAR(255),
        admit_type_code VARCHAR(50),
        admit_type_description VARCHAR(255)
        );""",
        """CREATE TABLE IF NOT EXISTS symptoms (
        patient_id VARCHAR(255),
        age VARCHAR(50),
        pathology VARCHAR(255),
        number_of_symptoms VARCHAR(50),
        fever_level VARCHAR(50),
        fatigue_level VARCHAR(50),
        rash_level VARCHAR(50),
        joint_pain_level VARCHAR(50)
        );"""]

        for schema in table_schemas:
            cur.execute(schema)

        # Commit changes
        conn.commit()

        # Close the connection
        cur.close()
        conn.close()





if __name__ == '__main__':
    main()
